-- Função dinâmica aprimorada para deleção de empresas incluindo autenticação
...
BEGIN
    ...
    -- Obter IDs de usuários relacionados para autenticação
    SELECT ARRAY_AGG(id) INTO user_ids
    FROM profiles
    WHERE company_id = target_company_id;
    
    RAISE NOTICE 'Usuários a serem tratados: %', user_ids;
    ...
    -- Deletar registros em cada tabela relacionada do esquema public
    ...
    LOOP
        BEGIN
            ...
            IF company_column IS NOT NULL THEN
                ...
                RAISE NOTICE 'Registros excluídos da tabela %: %', table_name, affected_rows;
            END IF;
    ...
    BEGIN
        ...
        GET DIAGNOSTICS affected_rows = ROW_COUNT;
        RAISE NOTICE 'Perfis de usuários atualizados (company_id removido): %', affected_rows;
    EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        ...
        GET DIAGNOSTICS affected_rows = ROW_COUNT;
        RAISE NOTICE 'Empresa excluída com sucesso: %', affected_rows;
    EXCEPTION WHEN OTHERS THEN
    ...
    -- Tratar especificamente as tabelas de autenticação (auth.users)
    -- Este passo é crucial pois auth.users está em outro esquema 
    -- e não seria detectado automaticamente pela busca de FKs
    IF array_length(user_ids, 1) > 0 THEN
        BEGIN
            -- Verificar usuários que não têm mais perfis
        ...
        EXCEPTION WHEN OTHERS THEN
            error_message := format('Erro ao excluir da tabela auth.users: %s', SQLERRM);
            RAISE WARNING '%', error_message;
            delete_success := false;
        END;
    END IF;
