...
interface MarcaSlidePanelProps {
  isOpen: boolean;
  onClose: () => void;
  marcaToEdit?: {
    id: string;
...
export function MarcaSlidePanel({ isOpen, onClose, marcaToEdit }: MarcaSlidePanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'Erro ao processar marca');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="h-full flex flex-col">
          <div className="p-6 border-b border-slate-700 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-slate-200">
              {marcaToEdit ? 'Editar Marca' : 'Nova Marca'}
            </h2>
            <button
              onClick={onClose}
